...
def get_epoched_values(
    st_unit, st_unit_rip, st_unit_no_rip, ep, ripple_epochs, state_epoch=False
):
    ...
    if state_epoch == False:
        unit_mat = get_participation(st_unit_rip[ep], ripple_epochs[ep])
        ...
        try:
            unit_mat = get_participation(
                st_unit_rip[ep][state_epoch], ripple_epochs[ep][state_epoch]
            )
            participation_prob = np.sum(unit_mat, axis=1) / unit_mat.shape[1]
            if len(participation_prob) == 0:
                participation_prob = np.nan
        ...
        try:
            avg_fr_not_rip = (
                st_unit_no_rip[ep][state_epoch].n_spikes
        ...
        try:
            avg_fr_in_rip = (
                st_unit_rip[ep][state_epoch].n_spikes
...
def main_analysis(
    ...
    for i, ep in enumerate(behavioral_epochs):
        ...
        (
            ...
            st_unit, st_unit_rip, st_unit_no_rip, ep, ripple_epochs, nrem_epochs
        )
        ...
        (
            ...
            st_unit, st_unit_rip, st_unit_no_rip, ep, ripple_epochs, wake_epochs
        )
