...
from ripple_heterogeneity.utils import functions,add_rem_shift
import nelpy as nel
import os
...
def get_group_cor_vectors(df,temp_df,basepath):
    ...
    for assembly_n in df[df.basepath == basepath].assembly_n.unique():
        ...
        deep.append(
            temp_df[(temp_df.layer_rem_shift_ref == 'deep_rem_shift') & 
            (temp_df.layer_rem_shift_target == 'deep_rem_shift')].rho.values
        )   
        sup.append(
            temp_df[(temp_df.layer_rem_shift_ref == 'sup_non_rem_shift') & 
            (temp_df.layer_rem_shift_target == 'sup_non_rem_shift')].rho.values
        ) 
        member_rho.append(
            temp_df[(np.in1d(temp_df.UID_ref,member_uid)) &
            (np.in1d(temp_df.UID_target,member_uid))].rho.values
            )
        non_member_rho.append(
            temp_df[(np.in1d(temp_df.UID_ref,nonmember_uid)) &
            (np.in1d(temp_df.UID_target,nonmember_uid))].rho.values
            )
        member_deep.append(
            temp_df[(np.in1d(temp_df.UID_ref,member_uid)) &
...
def get_pairwise_corrs(basepath):
    ...
    # add rem shift data
    cell_metrics = add_rem_shift.add_rem_shift(cell_metrics)
    ...
    temp_df['rho'] = rho
    temp_df['pval'] = pval
    temp_df['layer_rem_shift_ref'] = cell_metrics.layer_rem_shift.iloc[temp_df['ref']].values
    temp_df['layer_rem_shift_target'] = cell_metrics.layer_rem_shift.iloc[temp_df['target']].values
...
def assembly_corr_run(df,save_path,parallel=True):
    ...
    if parallel:
        ...
        processed_list = Parallel(n_jobs=num_cores)(delayed(session_loop)(basepath,df,save_path) for basepath in basepaths)
    else:    
