...
def get_replay_epochs(results, replay_type, direction, alpha=0.05):
    ...
    idx = (results[direction]["df"].score_pval_time_swap <= alpha) & (
        results[direction]["df"].replay_type == replay_type
    )
...
def get_pre_linear_post(basepath):
    ...
    return epoch_df, nel.EpochArray(
...
def run_all(basepath):
    ...
    epochs = {
        "forward_outbound_replay": get_replay_epochs(
            results, "forward", "outbound_epochs"
        ),
        "forward_inbound_replay": get_replay_epochs(
            results, "forward", "inbound_epochs"
        ),
        "reverse_outbound_replay": get_replay_epochs(
            results, "reverse", "outbound_epochs"
        ),
        "reverse_inbound_replay": get_replay_epochs(
            results, "reverse", "inbound_epochs"
        ),
    }
    ...
    n_replays = []
    n_active_replays = []
    par_mat = functions.get_participation(
        st.data, ripple_epochs.starts, ripple_epochs.stops
    )
    ...
    for beh_i, beh_epoch in enumerate(behavior_epochs):
        for key_ in epochs.keys():
            ...
            replay_particip.append(
                functions.get_participation(
                    st[beh_epoch].data, epochs[key_].starts, epochs[key_].stops
                ).mean(axis=1)
            )
