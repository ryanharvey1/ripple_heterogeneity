...
def run_all(basepath):
    ...
    par_mat = nel.AnalogSignalArray(
        data=par_mat, timestamps=ripple_epochs.centers)
    ...
    for beh_i, beh_epoch in enumerate(behavior_epochs):
        for key_ in epochs.keys():
            temp_rank_order, rank_order = functions.get_rank_order(
                st[beh_epoch], epochs[key_])
            if rank_order is None:
                continue
            median_rank_order.append(temp_rank_order)
            n_replays.append(
                np.tile(rank_order.shape[1], len(temp_rank_order)))
            n_active_replays.append(np.sum(rank_order > 0, axis=1))
            avg_fr.append(st[beh_epoch].n_spikes / beh_epoch.duration)
            replay_particip.append(functions.get_participation(
                st[beh_epoch].data, epochs[key_].starts, epochs[key_].stops).mean(axis=1))
    ...
    df_rank_order['label_lay'] = df_rank_order['label'] + \
        df_rank_order['deepSuperficial']
