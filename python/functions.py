...
def writeNeuroscopeEvents(path, ep, name):
    ...
    for i in range(len(ep)):
        f.writelines(
            str(ep.as_units('ms').iloc[i]['start']) + " "+name+" start " + str(1)+"\n")
...
def compute_psth(spikes, event, bin_width=0.002, n_bins=100):
    ...
    times = np.arange(0, bin_width*(n_bins+1), bin_width) - \
        (n_bins*bin_width)/2
...
def Otsu(vector):
    ...
    for i in np.arange(n):
        p = (i+1)/n
        p0 = 1 - p
        intraClassVariance[i] = p * \
            np.var(sorted[0:i+1]) + p0*np.var(sorted[i+1:])
...
def get_participation(st, event_starts, event_stops, par_type='binary'):
    ...
    unit_mat = get_participation_(fix_array_or_list(
        list(st)), event_starts, event_stops)
...
def find_laps(Vts, Vdata, newLapThreshold=15, good_laps=True,
    ...
    th1 = np.nanmin(Vdata) + TL*newLapThreshold / \
        100  # % lower threshold for lower end
    th2 = np.nanmax(Vdata) - TL*newLapThreshold / \
        100  # % upper threshold for upper end
...
def find_good_laps(ts, V_rest, laps, edgethresh=0.1, completeprop=0.2, posbins=50):
    ...
    bins = np.arange(bottomend, topend+(topend-bottomend) /
                     posbins, (topend-bottomend)/posbins)
    ...
    while l < len(laps)-1:
        ...
        t = ts[np.where(ts == laps.iloc[l].start_ts)[0][0]
                        :np.where(ts == endoflap)[0][0]]
...
def get_linear_track_lap_epochs(ts, x, newLapThreshold=15,
    ...
    outbound_epochs = nel.EpochArray(
        [np.array([outbound_start, outbound_stop]).T])
    inbound_epochs = nel.EpochArray(
        [np.array([inbound_start, inbound_stop]).T])
