...
# data$y_trans <- log(abs(data$corrcoef.1))
data$y_trans <- log(data$r2_pre+1)
...
```{r}
temp_data <- data[!is.nan(data$cor_pre),]
temp_data <- temp_data[!is.na(temp_data$cor_pre),]

temp_data$cor_pre_abs <- abs(temp_data$cor_pre)+1

mdl_boxcox <- boxcox(cor_pre_abs ~ region, data=temp_data, lambda = seq(-5, 10, length = 100))
max_lambda <- mdl_boxcox$x[max(mdl_boxcox$y) == mdl_boxcox$y]

temp_data$y_trans <- temp_data$cor_pre_abs^max_lambda

m1 = lmer(y_trans ~ region + (1 | animal_id/basepath/unique_uid_basepath), data = temp_data,REML=FALSE)
summary(m1)
...
plot(y_trans ~ region , data = temp_data)
```


```{r}
temp_data$cor_pre_abs
```
