...
```{r}
temp_data <- data[data$epoch == "post" & (data$label == "deep_mec" | data$label == "sup_mec") & !is.nan(data$mutual_info) & data$mutual_info != 0,]
temp_data$basepath = factor(temp_data$basepath)
temp_data$animal_id = factor(temp_data$animal_id)
temp_data$label = factor(temp_data$label)
...
# hist(temp_data$y_trans,100)
hist(temp_data$mutual_info,100)
hist(log(temp_data$mutual_info),100)
